...
if(isset($_GET['Pnumber'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		$del = mysqli_query($koneksi, "DELETE FROM project WHERE Pnumber='$Pnumber'") or die(mysqli_error($koneksi));
		if($del){
		...
		echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil4";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil4";</script>';
}
