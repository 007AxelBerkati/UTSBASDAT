...
if(isset($_GET['Dnumber'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		if($del){
			...
			echo '<script>alert("Gagal menghapus data."); document.location="index.php?page=tampil2";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil2";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil2";</script>';
}
