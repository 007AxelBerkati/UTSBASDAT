...
if(isset($_GET['Dlocation'])){
	...
	$cek = mysqli_query($koneksi, "SELECT * FROM dept_location WHERE Dlocation='$Dlocation'") or die(mysqli_error($koneksi));

	//jika query menghasilkan nilai > 0 maka eksekusi script di bawah
	if(mysqli_num_rows($cek) > 0){
		//query ke database DELETE untuk menghapus data dengan kondisi id=$id
		$del = mysqli_query($koneksi, "DELETE FROM dept_location WHERE Dlocation='$Dlocation'") or die(mysqli_error($koneksi));
		if($del){
